...
internal class TasksRepositoryMemoryTest {
    ...
    private lateinit var repository: TasksRepositoryMemory

    @BeforeEach
    fun init() {
        repository = TasksRepositoryMemory()
        repository.addTask(Task(1, "Task1", Priority.HIGH, true))
        repository.addTask(Task(2, "Task2", Priority.LOW))
        repository.addTask(Task(3, "Task3", Priority.MEDIUM, true))
        repository.addTask(Task(4, "Task4", Priority.HIGH))
        repository.addTask(Task(5, "Task5", Priority.MEDIUM))
    ...
    fun addTask() {
        ...
        assertEquals(newTask, repository.tasks.first { it.id == 6 })
    }
